...
copy_shared_pages(envid_t child)
{
	...
	for (i = 0; i < PDX(UTOP); i++) //Loop through PDir Entries	
	{
		if((uvpd[i] & PTE_P) && (uvpd[i] & PTE_U))
		{
			...
			{
				//cprintf("hello %d\n", i);					
				int pn = NPDENTRIES*i + j;
				uintptr_t addr = (uintptr_t) pn*PGSIZE;
				if((uvpt[pn] & PTE_SHARE) && (uvpt[pn] & PTE_U) && (uvpt[pn] & PTE_P))
				{
					//cprintf("here %d\n", i);					
					int permissions = uvpt[pn] & PTE_SYSCALL;
					if((r = sys_page_map(0, (void*) addr, child, (void*) addr, permissions)) < 0)
					{
						panic("Error %e when copying shared pages to child environment: %x, addr: %x\n", r, child, addr);
					}			
				}
			}	
		}
